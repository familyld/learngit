...
怎么理解保存工作现场的必要性呢？经过试验，如果我们在dev分支工作时没有commit，然后切换到master分支，新建Bug分支修改，这时工作区是脏的，实际上我们是在修改dev分支工作区的内容，而dev工作区的内容是未完成的，危险程度可想而知。 而且这种情况下切换会dev分支时，之前在dev工作区未提交的修改会丢失，只能回到dev分支上上一次commit过的版本，这就又要重新编写修改了。。。因此保存工作现场是非常重要的！！
