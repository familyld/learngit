...
第三行表示两个版本的Git哈希值（**index区域**(即上次commit的版本)的d9cd201对象，与**工作目录区域**(即当前修改过的内容)的85d8a04对象进行比较），最后的六位数字是对象的模式（普通文件，644权限）。
...
此外，使用 `git diff --cached` 可以对**暂存区**的版本和**index区域**的版本进行比较，也即上一次add的版本。
